...
void MapObject::Draw(SDL_Renderer* renderer)
{
	...
	case MObject::MOBJECT_TREE: 
		...
		this->hitbox[0].x = GRID_UNITS * ((gridNumber % CHUNK_UNITS)); 
		this->hitbox[0].y = GRID_UNITS * ((gridNumber / CHUNK_UNITS));
		this->hitbox[1].x = GRID_UNITS * ((gridNumber % CHUNK_UNITS) + 1);
		this->hitbox[1].y = GRID_UNITS * ((gridNumber / CHUNK_UNITS) + 1);
	...
	if (texture != nullptr)
	{
		float posX = MAP_START_X + tl.x - Camera::GetInstance()->GetPosition().x;
		float posY = MAP_START_Y + tl.y - Camera::GetInstance()->GetPosition().y;
		int sizeW = br.x - tl.x;
		int sizeH = br.y - tl.y;
