...
std::string GSMaze::formatStatus(int HP, int Hunger, int Thirst) {
	std::stringstream ss;
	ss << std::setfill(' ');
	ss << "   " << std::setw(3) << HP;
...
void GSMaze::UpdatePlayerStatus()
{
	//update status
	if (hungerDuration <= 0) {
...
void GSMaze::UpdateNearestObject()
{
...
void GSMaze::DisplayNearestObject(SDL_Renderer* renderer)
{
...
void GSMaze::GatherItem(MObject killedObj)
{
...
void GSMaze::UpdateStatus(float time)
{
